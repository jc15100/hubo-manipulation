    ...
    while (!daemon_sig_quit) {
        ...
        if (dt > 0) {
            ...
            if(cmd.controlMode == JOINT_VECTOR){
                ...
                // build transformation matrix for grasping object
                trans(0, 3) = trueGraspingPoint(0);
                trans(1, 3) = trueGraspingPoint(1);
                trans(2, 3) = trueGraspingPoint(2);

                // get current joint config
                hubo.getArmAngles(RIGHT, current);

                // get joing angles for target grasp
                hubo.huboArmIK(target, trans, current, RIGHT);
